                    ...
                    <tbody className="divide-y divide-gray-200">
                        {packages.map((pkg) => (
                            <tr key={pkg.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 text-sm font-medium text-gray-900">{pkg.title}</td>
                                <td className="px-6 py-4 text-sm text-gray-600">
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                        {pkg.category}
                                    </span>
                                </td>
                                <td className="px-6 py-4 text-sm text-gray-600">₹{pkg.startingPrice.toLocaleString()}</td>
                                <td className="px-6 py-4 text-right text-sm font-medium flex justify-end gap-3">
                                    <Link href={`/admin/packages/${pkg.id}`} className="text-blue-600 hover:text-blue-900 flex items-center gap-1">
                                        <Edit size={16} /> Edit
                                    </Link>
