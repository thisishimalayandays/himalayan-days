...
import { EnquiryModal } from '@/components/layout/enquiry-modal';
    ...
    return (
        <>
            <div className={`fixed right-6 z-50 flex items-end justify-end gap-3 pointer-events-none ${isPackageDetail ? 'bottom-24 md:bottom-6' : 'bottom-6'}`}>
                ...
                <motion.button
                    onClick={() => {
                        analytics.event('Contact', { content_name: 'WhatsApp Button Click' });
                        setIsModalOpen(true);
                    }}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="relative flex items-center justify-center pointer-events-auto group appearance-none border-none outline-none bg-transparent"
